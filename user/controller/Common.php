...
class Common extends Controller
{
    ...
    protected function checkUser()
    {
        ...
        if(!$this->chkip())
        {
            $this->redirect('admin/Common/quit');
            exit();
        }
        ...
        if(md5_file(CHCK_PATH .base64_decode('dXNlci9mb290ZXIuanM=')) != CHU)
    ...
    {
        if(stripos($picture,'>') === false && strpos($picture,'"') === false && strpos($picture,'\'') === false && stripos($picture,' on') === false && strpos($picture,';') === false && strpos($picture,'..') === false)
        {
